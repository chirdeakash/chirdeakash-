...
t1 = table(mtcars$cyl, mtcars$gear, dnn=c('cyl', 'gear'))#dnn gives dimension  of matrix
t1
table(mtcars$cyl)
table(mtcars$gear)
...
#sweep----
sweep(m1, MARGIN = 1, STATS = c(2,3,4), FUN="+" ) #rowise,it means add(2,3,4) to matrix rowise
sweep(m1, MARGIN = 2, STATS = c(2,3,4), FUN="*" ) #colwise,same as above
